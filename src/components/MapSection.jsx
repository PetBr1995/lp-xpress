function MapSection({ interactive = false, mapEmbedUrl = '', mapLinkUrl = '', locations = [] }) {
  ...
  return (
    <section className="border-t border-[#d0d4d8] bg-[radial-gradient(circle_at_25%_60%,#7db6cf_1px,transparent_2px),radial-gradient(circle_at_70%_30%,#c4d5e0_1px,transparent_2px),linear-gradient(90deg,#d9dfe4,#ebeff2)] px-6 py-8 [background-size:40px_40px,30px_30px,cover]" data-reveal="up">
      <div className="mx-auto w-full max-w-[960px]">
        <div className="mb-5 flex items-center justify-between gap-4 max-[680px]:mb-4 max-[680px]:flex-col max-[680px]:items-start">
          <h2 className="m-0 text-[38px] leading-[1.05] text-[#232860] max-[920px]:text-[28px]">Nossas unidades</h2>
          <span className="rounded-full bg-[#232860] px-3 py-1 text-[12px] tracking-[0.05em] text-white">ATENDIMENTO PRESENCIAL</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-[920px]:grid-cols-1">
          {normalizedLocations.map((location, index) => (
            ...
            >
              <header className="flex items-center justify-between border-b border-[#e8eaed] bg-white/95 px-4 py-3">
                <div>
                  <h3 className="m-0 text-[20px] text-[#232860]">{location.title || `Unidade ${index + 1}`}</h3>
