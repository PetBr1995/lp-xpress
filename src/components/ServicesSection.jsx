function ServicesSection({ services }) {
  return (
    <section
      id="servicos"
      className="mx-auto w-full max-w-[960px] px-6 pt-[58px] text-center"
      data-reveal="up"
    >
      ...
      <div className="grid grid-cols-4 gap-2 max-[920px]:grid-cols-2 max-[920px]:gap-4 max-[560px]:grid-cols-1">
        {services.map((service, index) => (
          <article
            key={service.title}
            className="min-h-[244px] rounded-lg bg-[#8c3294] px-4 py-5 text-left text-white"
            data-reveal="zoom"
      ...
      <button className="mt-[18px] rounded-md bg-[#222a67] px-5 py-2 text-[12px] text-white">
        Saiba mais
      </button>
    </section>
  );
